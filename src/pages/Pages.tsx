    ...
    <div className="flex-1 flex flex-col min-w-0">
      <header className="bg-white border-b border-gray-200 p-6 flex justify-between items-center sticky top-0 z-10">
        <div>
          ...
          <p className="text-sm text-gray-500">Define custom admin pages and navigation</p>
        </div>
        <button
      ...
      <div className="p-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {pages.map((page) => (
          <div key={page.id} className="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm hover:shadow-md transition-shadow group">
            <div className="flex justify-between items-start mb-4">
              <div className="bg-purple-50 p-3 rounded-xl text-purple-600">
                <Layers size={24} />
              </div>
              <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                <button onClick={() => handleOpenForm(page)} className="p-2 hover:bg-gray-100 rounded-lg text-gray-400 hover:text-blue-600">
                  <Edit2 size={16} />
                </button>
                <button onClick={() => handleDeleteClick(page.id)} className="p-2 hover:bg-gray-100 rounded-lg text-gray-400 hover:text-red-600">
                  <Trash2 size={16} />
                </button>
              </div>
            </div>
            <h3 className="text-lg font-bold text-gray-900 mb-1">{page.title}</h3>
            <p className="text-xs font-mono text-gray-400 mb-4">path: /p{page.path}</p>
          ...
          <div className="fixed inset-0 z-[100] bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
            ...
            >
              <div className="p-6 border-b border-gray-100 flex justify-between items-center">
                <h3 className="text-xl font-bold">{editingPage ? 'Edit Page' : 'New Custom Page'}</h3>
              ...
              <form onSubmit={handleSubmit} className="p-6 space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="space-y-1">
                    <label className="text-xs font-bold text-gray-500 uppercase tracking-wider">Path</label>
                    <input
                      required
                      value={formData.path}
                ...
                <div className="space-y-1">
                  <label className="text-xs font-bold text-gray-500 uppercase tracking-wider">Page Type</label>
                  <select
                    value={formData.type}
                ...
                <div className="bg-gray-50 p-4 rounded-xl border border-gray-100 space-y-4">
                  ...
                  {formData.type === 'static' && (
                    <div className="space-y-1">
                      <label className="text-xs font-bold text-gray-500 uppercase tracking-wider">Content (HTML/Markdown)</label>
                      <textarea
                  ...
                  {(formData.type === 'list' || formData.type === 'form') && (
                    <div className="space-y-1">
                      <label className="text-xs font-bold text-gray-500 uppercase tracking-wider">Select Data Model</label>
                      <select
                  ...
                  {formData.type === 'api-console' && (
                    <div className="space-y-1">
                      <label className="text-xs font-bold text-gray-500 uppercase tracking-wider">Select API Connection</label>
                      <select
